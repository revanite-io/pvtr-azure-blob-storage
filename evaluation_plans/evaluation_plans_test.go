...
// requirement ids in the vendored catalog(s) and the keys of the CCC plan
// map, so catalog upgrades cannot silently orphan either side. Keys outside
// the vendored catalogs' namespaces (e.g. CCC.Core.*, resolved from a
// reference catalog at runtime) are not checked here.
func TestPlanCoversCatalog(t *testing.T) {
	...
	for id := range catalogIDs {
		if _, ok := CCC[id]; !ok {
			t.Errorf("catalog requirement %s has no entry in the CCC plan map", id)
		}
	}
	for id := range CCC {
